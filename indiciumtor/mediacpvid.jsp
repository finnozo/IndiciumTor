	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="CSSTableGenerator">
     		 ...
     		 String sql="select * from department where DName='"+branch+"'";
			 String id="";
			 int bId;
            ...
            if(rs.next()){
            	...
            	sql="select * from suboffer where DID='"+id+"'";
            	rs = JdbcConnect.selectOperation(sql);
            	while(rs.next()){
            		String sqlsub ="select * from subject where id ="+rs.getInt("SID");
            		ResultSet rssub=JdbcConnect.selectOperation(sqlsub);
            		while(rssub.next()){
            			
            			String sqlvid="select * from videos where subject ='"+rssub.getString("subject")+"' and status = 0";
						ResultSet rsvid = JdbcConnect.selectOperation(sqlvid);
						while(rsvid.next()){
							%>
							
							<tr>
							<td><%out.println(rsvid.getString("name")); %></td>
							<td><video  controls="true" height="100" width="120">
								<source src="videoreader.jsp?id=<%=rsvid.getInt("id")%>" type="video/mp4" /> </video></td>
							<td><a href="aprovevid.jsp?vid=<%=rsvid.getString("id")%>&key=yes"> Yes Approved</a>&nbsp;&nbsp;&nbsp;<a href="aprovevid.jsp?vid=<%=rsvid.getString("id")%>&key=no"> Not Approved</a></td>
							</tr>
